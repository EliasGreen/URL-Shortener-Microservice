...
// get the model
let MyModel = mongoose.model('url', urlBranch);
...
app.get("/:ret", function (request, response) {
   MyModel.findOne({short_url: request.params.ret.toString()}, function (err, docs) {
      if(docs === null) {
...
let createRandomNumber = () => {
  ...
  MyModel.findOne({short_url: rndm.toString()}, function (err, docs) {
      if(docs === null) {
        short_URL = rndm.toString();
        // create a post
        let obj = {full_url: full_URL, short_url: short_URL};
        var post = new MyModel(obj);
